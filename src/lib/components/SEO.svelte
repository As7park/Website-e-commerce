<script lang="ts">
	...
	const finalTitle = $derived(
		title || (pageKey ? seoConfig.pages[pageKey].title : seoConfig.defaults.title)
	);
	const finalDescription = $derived(
		description || (pageKey ? seoConfig.pages[pageKey].description : seoConfig.defaults.description)
	);
	const finalKeywords = $derived(
		keywords || (pageKey ? seoConfig.pages[pageKey].keywords : seoConfig.defaults.keywords)
	);
	const finalImage = $derived(
		image || (pageKey ? seoConfig.pages[pageKey].image : seoConfig.defaults.image)
	);
	...
	const fullTitle = $derived(
		finalTitle === seoConfig.defaults.title ? finalTitle : `${finalTitle} | ${seoConfig.site.name}`
	);
...
<svelte:head>
	...
	<meta
		property="og:image"
		content={finalImage.startsWith('http') ? finalImage : `${seoConfig.site.url}${finalImage}`}
	/>
