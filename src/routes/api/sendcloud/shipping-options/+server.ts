...
// - Dedupe: keep only the cheapest quote per (carrier x type), across all carriers.
...
const InputZ = z
	.object({
		...
		dimensions: z.object({
			...
			unit: z.literal('cm').optional().default('cm') // seul appelant (checkout) : toujours en cm
		}),
...
function buildBasePayload(input: z.infer<typeof InputZ>) {
	...
	return {
		...
		// v3 `/shipping-options` (remplace `/fetch-shipping-options`, deprecated
		// depuis le 14/01/2026) attend un tableau `parcels` (multicolis) au lieu
		// d'un `weight`/`dimensions` à plat, et n'inclut les tarifs (`quotes`)
		// que si `calculate_quotes: true` est passé.
		parcels: [
			{
...
// Sendcloud `functionalities.last_mile` a 5 valeurs (pas juste service_point/home_delivery) :
// home_delivery, service_point, mailbox, locker, locker_or_service_point. Un point relais/casier
// (locker, locker_or_service_point) impose au client de se déplacer, comme service_point — à ne
// pas confondre avec mailbox (boîte aux lettres du domicile), qui reste une livraison à domicile.
const PICKUP_LAST_MILE = new Set(['service_point', 'locker', 'locker_or_service_point']);
function lastMileType(lastMile: string): 'service_point' | 'home_delivery' {
	return PICKUP_LAST_MILE.has(lastMile) ? 'service_point' : 'home_delivery';
}
...
function mapQuotesV3(resp: any): QuoteDTO[] {
	...
	for (const opt of arr) {
		...
		const carrierCode = String(opt?.carrier?.code ?? '')
			.toLowerCase()
			.trim();
		const productName = String(
			opt?.name ?? opt?.product?.name ?? opt?.product?.code ?? 'Unknown'
		).trim();

		const lastMile = String(opt?.functionalities?.last_mile ?? '').toLowerCase();
		const type: 'service_point' | 'home_delivery' = lastMileType(lastMile);
...
// Une seule proposition par transporteur x type (peu importe le produit) : évite le choix multiple au sein d'un même transporteur.
function familyKey(d: QuoteDTO): string {
	return `${d.carrierCode}|${d.type}`;
}
...
function extractImportantInfo(opt: any): ImportantInfo | null {
	...
	const type: 'home_delivery' | 'service_point' = lastMileType(lm);
...
export const POST: RequestHandler = async ({ request, fetch }) => {
	...
	const url = 'https://panel.sendcloud.sc/api/v3/shipping-options';
	const headers = {
